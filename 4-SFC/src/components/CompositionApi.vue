...
const quantityReposMessage = computed(() => {
    return state.repos.length > 0 ? `${state.name} possui ${state.repos.length} respositórios públicos`
...
<template>
    ...
    <button v-if="state.login !== ''" @click="fetchUserRepos">Carregar Repos</button>
    
    <section v-if="state.repos.length > 0" style="background-color: rgb(80, 77, 77);">
        <h2 v-if="state.login !== ''" style="background-color: black; font-size: 22px;">{{ quantityReposMessage }}</h2>
